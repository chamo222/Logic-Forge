...
export const loginUser = async (
    ...
    try {
        ...
        setCookie(res, "refresh_token", refrechToken);
...
export const refreshToken = async (
    ...
    try {
        ...
        if (!refrechToken) {
            return new ValidationError("Unauthorizd! No refresh token.");
        }
...
export const verifySeller = async(
    ...
    try {
        ...
        res
            .status(200)
            .json({ seller, message: "Seller registerd successfully!" });
    } catch (error) {
       next(error); 
    }
};
...
export const loginSeller = async (
    ...
    try {
        ...
        res.status(200).json({
            message: "Login successful!",
            seller: { id: seller.id, email: seller, name: seller.name },
        });
    } catch (error) {
        next(error);
    }
